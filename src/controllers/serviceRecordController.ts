...
export const getVehicleServices = asyncWrapper(async (req: Request, res: Response) => {
  const records = await serviceRecordService.getServiceRecordsByVehicle(req.params.id);
  res.status(200).json({ success: true, data: records });
});

export const addVehicleService = asyncWrapper(async (req: Request, res: Response) => {
  const record = await serviceRecordService.createServiceRecord(req.params.id, req.body);
